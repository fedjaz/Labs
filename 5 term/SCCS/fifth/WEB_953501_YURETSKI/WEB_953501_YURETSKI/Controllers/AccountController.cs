...
namespace WEB_953501_YURETSKI.Controllers
{
    public class AccountController : Controller
    {
        ...
        {
            string base64Avatar = ImageToBase64(Avatar);
        ...
        public async Task<IActionResult> GetAvatar()
        {
            ...
            if (signInManager.IsSignedIn(User))
            {
                ...
                return File(new MemoryStream(Convert.FromBase64String(base64Avatar)), "image/png");
            }
            else
            {
                return defaultAvatar;
        ...
        public async Task<IActionResult> Register(RegisterViewModel model)
        {
            ...
            {
                string base64avatar = ImageToBase64(model.Avatar);
        ...
        public string ImageToBase64(IFormFile formFile)
        {
            try
            {
                Stream stream = formFile.OpenReadStream();
                Image image = Image.FromStream(stream);
                Bitmap bitmap = new Bitmap(Math.Min(image.Width, image.Height), Math.Min(image.Width, image.Height));
