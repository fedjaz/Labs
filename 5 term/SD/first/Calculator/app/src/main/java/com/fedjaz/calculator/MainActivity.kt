...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val activation = Activation(applicationContext)
        isDemo = activation.isDemo()

        setListeners()
        showLess()

        if(!isDemo && !activation.checkActivation()){
            val newIntent = Intent(this, ActivationActivity::class.java)
    ...
    private fun setListeners() {
        ...
        if(isDemo){
            lessButton.setOnClickListener {
                Toast.makeText(applicationContext, "Scientific calculator is available at full version", Toast.LENGTH_LONG).show()
                vibrator.vibrate(25)
            }
            lessButton.addOnLayoutChangeListener { _, _, _, _, _, _, _, _, _ ->
                setCompatibleImage(lessButton, R.mipmap.moreless_demo_foreground, 0.6)
            }
        }
        else{
            lessButton.setOnClickListener {
