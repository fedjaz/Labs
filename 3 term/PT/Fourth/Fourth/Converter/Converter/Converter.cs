...
namespace Converter
{
    public class Converter : IParser
    {
        ...
        T Deserialize<T>(List<ParsedObject> objects)
        {
            T ans;
            Type type = typeof(T);
            ...
            foreach(ParsedObject parsedObject in objects)
            {
                ...
                {
                    object parsed = typeof(Converter).GetMethod("Deserialize", BindingFlags.NonPublic | BindingFlags.Instance)
                                                     .MakeGenericMethod(memberType)
                                                     .Invoke(this, new object[] { ParseJson(value) });
                ...
                else if(parsedObject.Type == ParsedObject.ObjectType.XmlComplex)
                {
                    ...
                    else
                    {
                        object parsed = typeof(Converter).GetMethod("Deserialize", BindingFlags.NonPublic | BindingFlags.Instance)
                                                     .MakeGenericMethod(memberType)
                                                     .Invoke(this, new object[] { ParseXML(value, false) });
                        SetMemberValue(ans, key, parsed);
                    }
        ...
        IList ParseArray(Type type, ParsedObject obj)
        {
            ...
            foreach(string value1 in obj.ValueArray)
            {
                list.Add(typeof(Converter).GetMethod(methodName)
                                          ...
                                          .Invoke(this, new object[] { value1.Trim() }));
            }
            return list;
        }
        ...
        void SetArrayValue(object obj, string memberName, ParsedObject parsedObject)
        {
